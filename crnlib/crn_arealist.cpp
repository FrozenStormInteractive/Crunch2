...
namespace crnlib
{
    ...
    Area_List* Area_List_dup_new(Area_List* Plist, int x_ofs, int y_ofs)
    {
        int i;
    ...
    void Area_List_dup(Area_List* Psrc_list, Area_List* Pdst_list, int x_ofs, int y_ofs)
    {
        int i;
    ...
    void Area_List_copy(Area_List* Psrc_list, Area_List* Pdst_list, int x_ofs, int y_ofs)
    {
        ...
        else
        {
            ...
            while (Parea != Psrc_list->Ptail)
            {
                insert_area_after(Pdst_list, Pdst_list->Phead, Parea->x1, Parea->y1, Parea->x2, Parea->y2);
                Parea = Parea->Pnext;
            }
#endif
    ...
    void Area_List_remove(Area_List* Plist, int x1, int y1, int x2, int y2)
    {
        int l, h;
        Area* Parea = Plist->Phead->Pnext;
        ...
        {
            // Not touching
            if ((x2 < Parea->x1) || (x1 > Parea->x2) || (y2 < Parea->y1) || (y1 > Parea->y2))
            {
                Parea = Parea->Pnext;
                continue;
            }
            ...
            // Completely covers
            if ((x1 <= Parea->x1) && (x2 >= Parea->x2) && (y1 <= Parea->y1) && (y2 >= Parea->y2))
            {
                if ((x1 == Parea->x1) && (x2 == Parea->x2) && (y1 == Parea->y1) && (y2 == Parea->y2))
                {
            ...
            if (y1 > Parea->y1)
            {
                insert_area_before(Plist, Parea, Parea->x1, Parea->y1, Parea->x2, y1 - 1);
            }

            // bottom
            if (y2 < Parea->y2)
            {
                insert_area_before(Plist, Parea, Parea->x1, y2 + 1, Parea->x2, Parea->y2);
            }
            ...
            if (x1 > Parea->x1)
            {
                insert_area_before(Plist, Parea, Parea->x1, l, x1 - 1, h);
            }

            // right middle
            if (x2 < Parea->x2)
            {
                insert_area_before(Plist, Parea, x2 + 1, l, Parea->x2, h);
            }
    ...
    void Area_List_insert(Area_List* Plist, int x1, int y1, int x2, int y2, bool combine)
    {
        Area* Parea = Plist->Phead->Pnext;
    ...
    void Area_List_intersect_area(Area_List* Plist, int x1, int y1, int x2, int y2)
    {
        Area* Parea = Plist->Phead->Pnext;
        ...
        while (Parea != Plist->Ptail)
        {
            ...
            insert_area_before(Plist, Parea,
                math::maximum(x1, Parea->x1), math::maximum(y1, Parea->y1),
                math::minimum(x2, Parea->x2), math::minimum(y2, Parea->y2));
...
#if 0
    void Area_List_intersect_Area_List(Area_List* Pouter_list, Area_List* Pinner_list, Area_List* Pdst_list)
    {
        ...
        while (Parea1 != Pouter_list->Ptail)
        {
            ...
            {
                if ((x1 <= Parea2->x2) && (x2 >= Parea2->x1) && (y1 <= Parea2->y2) && (y2 >= Parea2->y1))
                {
                    insert_area_after(Pdst_list, Pdst_list->Phead,
                        math::maximum(x1, Parea2->x1), math::maximum(y1, Parea2->y1),
                        math::minimum(x2, Parea2->x2), math::minimum(y2, Parea2->y2));
                }
        ...
        while (Parea1 != Pouter_list->Ptail)
        {
            ...
            while (Parea2 != Pinner_list->Ptail)
            {
                if ((x1 <= Parea2->x2) && (x2 >= Parea2->x1) && (y1 <= Parea2->y2) && (y2 >= Parea2->y1))
                {
                    ...
                    {
                        insert_area_after(Pdst_list, Pdst_list->Phead, nx1, ny1, nx2, ny2);
                    }
                    else
                    {
    ...
    Area_List_Ptr Area_List_create_optimal(Area_List_Ptr Plist)
    {
        ...
        while (Parea != Plist->Ptail)
        {
            Parea_after = insert_area_after(Pnew_list, Parea_after, Parea->x1, Parea->y1, Parea->x2, Parea->y2);
